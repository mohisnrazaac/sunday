...
class TermController extends Controller
{
    ...
    public function index()
    {
       ...
       // ->where('course_id', 3)->get();
        return $this->service->view('index', compact('terms'));
    }

    /**
     * Display a listing of the resource with course id.
     *
     * @return \Illuminate\Contracts\View\View|\Illuminate\Contracts\View\Factory
     */
    public function gettermsbycourse($course_id)
    {
        $this->authorize('term.index');
        //$terms = $this->service->index();
        $terms=DB::table('terms')
        ->select('courses.title as courseTitle','departments.title as departmentTitle','terms.title as termsTitle','terms.id as termsId')
        ->join('courses', 'courses.id', '=', 'terms.course_id')
        ->join('departments','departments.id','=','terms.department_id')
        ->where('course_id', $course_id)->get();

        //print_r($terms);exit;
        return $this->service->view('gettermscourse', compact('terms'));
    }
